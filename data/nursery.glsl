...
mat3 m = mat3( 1.00,  5.90,  0.60,
              -0.90,  0.36, -0.48,
              -0.60, -0.48,  0.34 );
float time = iGlobalTime+94.0;
...
//      vec2 mo = (-1.0 + 2.0 + 20.0) / iResolution.xy;  
    // Camera code...
    vec3 ro = 5.6*normalize(vec3(cos(2.75-1.0*mo.x), .4-1.3*(mo.y-2.4), sin(2.75-2.0*mo.x)));
